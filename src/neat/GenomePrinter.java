...
public class GenomePrinter {
    ...
    public static void printGenome(Genome genome, String path) {
        ...
        for (Entry<Integer, NodeGene> entry : genome.nodeGenes.entrySet()) {
            NodeGene gene = entry.getValue();
            if (null != gene.type) {
                switch (gene.type) {
                    case INPUT: {
                        float x = ((float) (entry.getKey() + 1) / ((float) countNodesByType(genome, NodeType.INPUT) + 1f)) * imageSize;
                        float y = imageSize - nodeSize / 2;
                        g.fillOval((int) (x - nodeSize / 2), (int) (y - nodeSize / 2), nodeSize, nodeSize);
                        nodeGenePositions.put(entry.getKey(), new Point((int) x, (int) y));
                        break;
                    }
                    case HIDDEN: {
        ...
        for (ConnectionGene gene : genome.connectionGenes.values()) {
            ...
            g.setStroke(new BasicStroke((float) (3.0 * Math.abs(gene.weight))));
        ...
        g.setColor(Color.WHITE);
        for (Entry<Integer, NodeGene> entry : genome.nodeGenes.entrySet()) {
            Point p = nodeGenePositions.get(entry.getKey());
    ...
    private static int countNodesByType(Genome genome, NodeType type) {
        int c = 0;
        for (NodeGene node : genome.nodeGenes.values()) {
            if (node.type == type) {
                c++;
            }
        }
